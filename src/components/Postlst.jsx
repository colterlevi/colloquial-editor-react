        ...
        <div className="flex-col w-full h-auto overflow-auto scrollbar-hide md:scrollbar-default">
            <div className="flex-row bg-cello m-10 p-5 rounded-lg">
                <Outlet />
                {
                    posts.map((post) => {
                        return (
                            <div key={post.id} className="w-auto h-auto my-8 p-5 rounded-lg bg-slate hover:bg-chateau hover:text-tamarillo hover:cursor-pointer" onClick={() => { navigate(`edit/${post.id}`) }}>
